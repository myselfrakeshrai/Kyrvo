...
const WebSchema: z.ZodType<Web> = z.object({
  ChiefMessageImage: z.string().optional(),
});

const WebChiefMessagePage: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<Web> = (formData: Web) => {
    if (id) {
      const confirm = window.confirm(
        `This will update Web "${defaultValues?.Id}" in production. Do you want to continue?`,
      );
      ...
      if (confirm) {
        ...
        WebServices.edit(id || '0', formData)
          ...
          .finally(() => {
            setLoading(false);
            navigate('/manage/webchiefMessage');
          });
      }
    } else {
      setLoading(true);
    ...
    <Grid>
      <Grid container alignItems="center" spacing={2}>
        <Grid item xs={6}>
          ...
          >
            {PAGE_ACTIONS.edit} Web ChiefMessage
          </Typography>
        </Grid>
      </Grid>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={4} marginTop={1}>
          <Grid item sm={6}>
            <ImageSelector
              label="ChiefMessageImage1"
              singleSelect={true}
              initial={
                getValues('ChiefMessageImage')
                  ? getValues('ChiefMessageImage')?.split('|')
                  : defaultValues?.ChiefMessageImage?.split('|') || []
              }
              onSubmit={(ids: string[]) => {
                setValue('ChiefMessageImage', ids.join('|'), {
                  shouldDirty: true,
                });
              }}
            />
...
export default WebChiefMessagePage;
