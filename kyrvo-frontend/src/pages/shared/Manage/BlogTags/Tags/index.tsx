...
const BlogTagSchema: z.ZodType<BlogTag> = z.object({
...
const BlogTagPage: React.FC = () => {
  ...
  const {
    ...
    defaultValues: async () => (id ? BlogTagService.get(id) : ({} as BlogTag)),
  });

  const onSubmit: SubmitHandler<BlogTag> = (formData: BlogTag) => {
    if (id) {
      ...
      if (confirm) {
        setLoading(true);
        BlogTagService.edit(id || '0', formData)
          .then((res) => {
            return res;
          })
          .catch((e) => {
      ...
      setLoading(true);
      BlogTagService.addNew(formData)
        .then((res) => {
          goto('blogtags');
          return res;
        })
