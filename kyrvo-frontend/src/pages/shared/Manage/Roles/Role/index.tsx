...
const RoleSchema: z.ZodType<Role> = z.object({
  Id: z.string().min(3, 'Role Id must be at least 3 characters long.'),
...
const RolePage: React.FC = () => {
  ...
  const {
    ...
    defaultValues: async () =>
      id ? RoleServices.get(id) : ({ Name: '', Desc: '' } as Role),
    resolver: zodResolver(RoleSchema),
  });

  const onSubmit: SubmitHandler<Role> = (formData: Role) => {
    if (id) {
      ...
      if (confirm) {
        setLoading(true);
        RoleServices.edit(id || '0', formData)
          .then((res) => {
            return res;
          })
          .catch((e) => {
